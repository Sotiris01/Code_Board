...
class GridEditor {
    ...
    _getSyntaxHighlighting() {
        // Returns array of arrays: highlightData[row][col] = 'syntax-class'
        // Uses GLOSSA_KEYWORDS if available
        
        if (typeof GLOSSA_KEYWORDS === 'undefined') {
            return [];
        }
        ...
        for (let row = 0; row < this.lines.length; row++) {
            ...
            if (trimmedLine.startsWith('!')) {
                ...
                for (let col = 0; col < line.length; col++) {
                    lineClasses[col] = 'syntax-comment';
                }
            ...
            this._highlightLine(line, lineClasses);
    ...
    _highlightLine(line, lineClasses) {
        let pos = 0;
        ...
        while (pos < line.length) {
            ...
            if (/[α-ωά-ώΑ-ΩΆ-Ώa-zA-Z_]/.test(line[pos])) {
                ...
                const syntaxClass = this._getKeywordClass(word);
